...
{
    [SerializeField] private float horizontalAmount; // 구름 이동 범위
    [SerializeField] private float horizontalSpeed; // 구름 이동 속도
    ...
    private void Update()
    {
        float x = Mathf.Sin(Time.time * horizontalSpeed) * horizontalAmount; // 시간에 따라 x 좌표 계산
        float y = Mathf.Sin(Time.time * verticalSpeed) * verticalAmount; // 시간에 따라 y 좌표 계산

        transform.localPosition = startlocalPosition + new Vector3(x, y, 0f); // 구름의 위치 업데이트
    }
}
